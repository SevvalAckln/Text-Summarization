...
def select_file():
    filename = askopenfilename()# Dosya seçme iletişim kutusunu aç
    
    label.config(text=filename)# Seçilen dosya yolunu etikete yazdır
    
    with open(filename, 'r') as file:# Dosya içeriğini oku

        contents= file.read()
        sentences = contents.split('.')# Cümleleri parçala ve listeye ekle
        print(sentences)
        ...
        G = nx.Graph()# Boş bir graf oluştur
        ...
        pos = nx.spring_layout(G) # Grafiği çiz
        nx.draw(G, pos, with_labels=True)

        labels = {} # Her düğümün etiketi olarak cümleyi göster
        for node in G.nodes():
            labels[node] = node
    ...
    # Cümle içerisinde özel isimleri bul, p1 parametresi burada yapıldı///////////////////////////////////////////////////////////////////////////////////////      (P1)
    ozel_isim_sayisi = 0
    for cumle in cumleler:
        words = nltk.word_tokenize(cumle)
        tagged = nltk.pos_tag(words)
        for word, tag in tagged:
            if tag == 'NNP':#eğer özel isimse (bunun için nltk içerisinde hazır bulunan NNP'den yararlanıyoruz.)
                ozel_isim_sayisi += 1
            
    if ozel_isim_sayisi != 0:#eğer özel isim varsa bir cümle içerisinde
    ...
    # Cümle içerisinde nümerik veri  bul, p2 parametresi burada yapıldı////////////////////////////////////////////////////////////////////////////////////////     (P2)
    ...
    sentences = nltk.sent_tokenize(contents)# cümleleri parçala ve listeye ekle
    baslik = []
    
    for sentence in sentences:# her bir cümle için başlık kelime sayısını hesapla
    ...
    word_freq = Counter(words) # kelime frekansları hesaplanıyor
    
    num_keywords = int(word_count * 0.1)# dokümandaki kelime sayısının %10'u hesaplanıyor
    
    keywords = [w for w, _ in word_freq.most_common(num_keywords)]# en sık geçen num_keywords kelime belirleniyor
    
    sentences = nltk.sent_tokenize(contents)# cümleleri parçala ve listeye ekle

    
    ratios = []# her bir cümle için temel kelime sayısı hesapla
    for sentence in sentences:
        words = nltk.word_tokenize(sentence)
...
#burada kendi oluşturduğumuz Cümle skorunu kullanıyo oluşturduğumuz özetmee algoritması 
    kelimelersay = {}
    cumlelersay = {}
    tekrar =[]
    ozet =""
...
# kelimelerin toplamda kaç tane geçtiğini sayıyor. Her kelimeye tekrarına göre puan veriyor.
    ...
    wordEmbedding("C:/Users/NSevvalAcikalin/Desktop/girdi.txt","C:/Users/NSevvalAcikalin/Desktop/cumleVektor.txt","C:/Users/NSevvalAcikalin/Desktop/kosinus.txt")
    rouges()
